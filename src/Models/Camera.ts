...
export class Camera {
  ...
  public update(delta: number) {
    if (this.thirdTarget && this.activeCtrl === this.thirdCtrl) {
      ...
      this.thirdCtrl.setTarget(
        Constants.__OUT_WORLD__POS.x,
        Constants.__OUT_WORLD__POS.y,
        Constants.__OUT_WORLD__POS.z,
        true
      );
    }
  ...
  public setFollowTarget(target: SystemObject) {
    ...
    this.thirdCtrl.setPosition(
      Constants.__OUT_WORLD__POS.x,
      Constants.__OUT_WORLD__POS.y,
      Constants.__OUT_WORLD__POS.z + target.data.drawRadius * 4
    );
