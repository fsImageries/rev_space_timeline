...
void main() {
    vec4 p = vec4(vPosition*1000., time * 2.);
    float noisy = fbm(p);

    vec4 p1 = vec4(vPosition*102., time *2.);
    float spots = max(snoise(p1), 0.) / 2.;

    //gl_FragColor = vec4(0.,0.,0.,1.0) + vec4(noisy);
    //gl_FragColor = vec4(noisy);
    //gl_FragColor *= mix(1., spots, 0.7);
    
    //vec3 col1 = bright2Col(noisy);
    float fres = Fresnel(eyeVector, vNormal);

    noisy += fres;
    spots += fres;
    vec3 col1 = vec3(noisy, 0., 0.);
    vec3 col2 = vec3(spots, 0., 0.);
